...
3. The workflow dispatches pull-request metadata and version guard against the exact
   ...
   a successfully verified pull-request head and every manifest/lock change is the exact
   old-version-to-new-version substitution generated by the release process.
