...
use crate::command::helpers::memory::{
...
pub(crate) fn execute_memory(arguments: &[String]) -> Result<CliOutput, String> {
    ...
    if help::requested(&arguments) {
        return Ok(CliOutput::success(help::text(&arguments)));
    }
    let parsed = parsing::parse(&arguments)?;
...
fn execute(
    ...
    match command {
        ...
        MemoryCommand::Rebuild => {
            ...
            Ok(CliOutput::success(human_rendering::rebuild(
                &summary,
                &project.repository_root,
                &project.database_path,
                color,
...
fn schema(relation: Option<&str>, color: bool) -> Result<CliOutput, String> {
    if let Some(name) = relation {
        ...
        return Ok(CliOutput::success(human_rendering::relation(
            &relation, color,
        )));
    }
    Ok(CliOutput::success(human_rendering::schema(
        &memory_schema(),
        color,
    )))
}
...
fn graph(
    ...
    let stdout = if machine {
        structured_rendering::graph_json(&result, query)
    } else {
        graph_rendering::graph(&result, query, color)
    };
    Ok(CliOutput {
        stdout,
...
fn render_query(result: &MemoryQueryResult, format: &str, color: bool) -> String {
    match format {
        QUERY_FORMAT_JSON => structured_rendering::query_json(result),
        QUERY_FORMAT_CSV => structured_rendering::query_csv(result),
        QUERY_FORMAT_TABLE => human_rendering::query_table(result, color),
        _ => human_rendering::query_long(result, color),
    }
}
