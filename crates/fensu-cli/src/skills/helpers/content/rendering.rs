...
fn cacheability_lines(context: &SkillContext) -> Vec<String> {
    ...
    if selected_custom {
        ...
        text.push_str("Cacheable custom rules use only allowlisted pure imports; never call `open`, `eval`, `exec`, `input`, or `__import__`; perform no direct filesystem access; make every cross-file query through `ctx.project` with `requester=ctx.path`; emit deterministic diagnostics; and keep helpers inside configured rule packages.\n\n");
    }
