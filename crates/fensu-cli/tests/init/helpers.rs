...
pub(crate) fn concurrent_fensu_and_editor_changes_fail_closed() {
    ...
    let repository = tempfile::tempdir().expect("concurrent target repository");
    ...
    let child = Command::new(env!("CARGO_BIN_EXE_fensu"))
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("concurrent target process");
    ...
    let output = child.wait_with_output().expect("concurrent result");
