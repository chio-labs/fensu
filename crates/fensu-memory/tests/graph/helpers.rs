...
use fensu_memory::graph::main::resolve_memory_graph::resolve_memory_graph;
use fensu_memory::graph::models::MemoryGraph;
...
pub(crate) fn load_graph(files: &[FixtureFile]) -> (PathBuf, MemoryGraph) {
    ...
    let root =
        std::env::temp_dir().join(format!("fensu-memory-graph-{}-{index}", std::process::id()));
