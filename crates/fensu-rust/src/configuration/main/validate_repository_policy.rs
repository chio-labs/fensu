...
pub(crate) fn validate_repository_policy(
    ...
    for (name, paths) in [
        ...
        if let Some(path) = paths.iter().find(|path| !valid_repository_path(path)) {
            return Err(format!(
                "Rust rule policy {name} must be repository-relative POSIX paths: {path}"
            ));
        }
    }
    for (index, path) in repository.intentional_layout_paths.iter().enumerate() {
        ...
        {
            return Err(format!(
                "Rust rule policy intentional layout paths must not overlap: {path}"
            ));
        }
        if repository
            .domain_paths
        ...
        {
            return Err(format!(
                "Rust rule policy intentional layout path overlaps a declared structural path: {path}"
            ));
        }
    }
    ...
    .contains(&0)
    {
        return Err("Rust rule policy thresholds must be greater than zero".to_owned());
    }
    Ok(())
}
