...
fn crate_name_violations(
    ...
    let mut violations = crates
        ...
        .map(|workspace_crate| {
            ...
            models::Violation::new(models::ViolationRequest {
                ...
                remediation:
                    "add the package to rule_options.FPRSL304.crate_names or remove it from the workspace",
            })
        })
        .collect::<Vec<_>>();
    for configured in &config.crate_names {
        ...
        violations.push(models::Violation::new(models::ViolationRequest {
            ...
            remediation:
                "remove the stale rule_options.FPRSL304.crate_names entry or add the package to the workspace",
        }));
    }
    violations
}
...
fn inventory_violation(path: &str, kind: &str, stale: bool) -> models::Violation {
    models::Violation::new(models::ViolationRequest {
        ...
        remediation: if stale {
            format!("remove {path} from rule_options.FPRSL305.{kind}_paths or restore its Rust structure")
        } else {
            format!(
                "add {path} to rule_options.FPRSL305.{kind}_paths or remove the structural path"
            )
        },
    })
}
