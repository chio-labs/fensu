//! Shared temporary-repository helpers for Rust analyzer tests.
...
use fensu_rust::constants;

static REPO_COUNTER: atomic::AtomicUsize = atomic::AtomicUsize::new(0);

pub(crate) fn check_repository(root: &path::Path) -> Vec<fensu_rust::models::Violation> {
    fensu_rust::rules::main::check_repository_with_config::check_repository_with_config(
        root,
        &fensu_rust::models::RustPolicy::default(),
    )
    .expect("default Rust policy is valid")
}
...
fn write_repo(test_case: &test_types::CheckRepoTestCase) -> path::PathBuf {
    ...
    let root = std::env::temp_dir().join(format!("example-tooling-{}-{index}", std::process::id()));
...
pub(crate) fn collect_violation_codes(repo_root: &path::Path) -> Vec<&'static str> {
    ...
    .expect("fixture rule policy is valid")
    .iter()
...
pub(crate) fn tooling_entry(domain: &str, module: &str) -> test_types::RepoFile {
    test_types::RepoFile {
        path: format!("crates/example-tooling/src/{domain}/main/{module}.rs"),
