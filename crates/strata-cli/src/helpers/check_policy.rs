...
pub(crate) fn required_thresholds(codes: &[String]) -> HashSet<&'static str> {
    ...
    for code in codes {
        match code.as_str() {
            "SFR301" => names.extend(["max_helpers_container_modules", "max_role_depth"]),
            "SFR302" => names.extend(["max_main_container_modules", "max_role_depth"]),
            "SFR308" => {
...
pub(crate) fn check_identity(config: &Config, sources: &[ScopedSource], warnings: bool) -> String {
    ...
    digest.update(b"strata-native-check-v1\0");
