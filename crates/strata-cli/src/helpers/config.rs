...
pub(crate) fn load(start: &Path) -> Result<(PathBuf, Config), String> {
    ...
    let value = if pyproject {
        document
            .get("tool")
            .and_then(|value| value.get("strata"))
            .ok_or_else(|| format!("{} does not contain [tool.strata].", path.display()))?
    } else {
        &document
    };
    ...
    Ok((
        ...
        Config {
            ...
            select: strings_or(table.get("select"), &["SF"]),
...
fn find(start: &Path) -> Result<(PathBuf, bool), String> {
    ...
    for directory in resolved.ancestors() {
        let strata = directory.join("strata.toml");
        if strata.is_file() {
            return Ok((strata, false));
        }
        let pyproject = directory.join("pyproject.toml");
        if pyproject.is_file() {
            let text = fs::read_to_string(&pyproject).unwrap_or_default();
            if text.contains("[tool.strata]") {
                return Ok((pyproject, true));
            }
        }
    }
    Err("Could not find strata.toml or [tool.strata] in pyproject.toml.".to_owned())
}
