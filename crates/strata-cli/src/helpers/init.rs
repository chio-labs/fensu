...
const STRATA_IGNORE: &str = "# Strata\n.strata/cache/\n";
    ...
    while index < arguments.len() {
        match arguments[index].as_str() {
            ...
            "--root" | "--tests" | "--tooling" => {
                ...
                if values.is_empty() {
                    return Err(format!(
                        "strata init: error: argument {option}: expected at least one argument"
                    ));
                }
                match option.as_str() {
            ...
            value => {
                return Err(format!(
                    "usage: strata init ...\nstrata init: error: unrecognized arguments: {value}"
                ))
            }
        }
        index += 1;
...
fn required_value(arguments: &[String], index: usize, option: &str) -> Result<String, String> {
    arguments
        ...
        .cloned()
        .ok_or_else(|| format!("strata init: error: argument {option}: expected one argument"))
}

fn local_config(repository: &Path) -> Option<PathBuf> {
    let strata = repository.join("strata.toml");
    if strata.is_file() {
        return Some(strata);
    }
    ...
    fs::read_to_string(&pyproject)
        .ok()
        .filter(|text| text.contains("[tool.strata]"))
        .map(|_| pyproject)
}
...
fn write_config(
    ...
    text.push_str("select = [\"SF\"]\n");
    fs::write(repository.join("strata.toml"), text).map_err(|error| error.to_string())
}
...
fn write_gitignore(repository: &Path, empty: bool) -> Result<(), String> {
    ...
    if !String::from_utf8_lossy(&value).contains(".strata/cache/") {
        value.extend_from_slice(STRATA_IGNORE.as_bytes());
    }
