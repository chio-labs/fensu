...
pub(crate) fn run_python(arguments: &[String]) -> Result<i32, String> {
    ...
    let status = Command::new(python_executable()?)
        ...
        .args(arguments)
        .status()
        .map_err(|error| format!("Could not launch Strata's Python command host: {error}"))?;
    Ok(exit_code(status))
}
...
pub(crate) fn verify_authoring_version() -> Result<(), String> {
    let Some(version) = installed_authoring_version() else {
        return Err(
            "stratalint is not installed beside stratalint-cli; install `stratalint` or use the CLI package only for `--version`."
                .to_owned(),
        );
    };
    if version != env!("CARGO_PKG_VERSION") {
        return Err(format!(
            "stratalint-cli {} does not match installed stratalint {version}. Upgrade both packages together with `python -m pip install --upgrade stratalint`. ",
...
fn metadata_version(site_packages: &Path) -> Option<String> {
    for entry in site_packages.read_dir().ok()?.flatten() {
        ...
        if !name.starts_with("stratalint-") || !name.ends_with(".dist-info") {
            continue;
        }
...
fn python_executable() -> Result<PathBuf, String> {
    if let Some(value) = env::var_os("STRATA_PYTHON") {
