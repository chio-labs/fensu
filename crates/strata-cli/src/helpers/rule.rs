...
fn parse_arguments(arguments: &[String]) -> Result<(String, String), String> {
    ...
    while index < arguments.len() {
        if arguments[index] == OPTION_COLOR {
            ...
            color = arguments.get(index).cloned().ok_or_else(|| {
                "strata rule: error: argument --color: expected one argument".to_owned()
            })?;
            if !matches!(color.as_str(), COLOR_AUTO | COLOR_ALWAYS | COLOR_NEVER) {
                return Err(format!("strata rule: error: invalid choice: '{color}'"));
            }
        } else if arguments[index].starts_with('-') {
            return Err(format!(
                "strata rule: error: unrecognized arguments: {}",
                arguments[index]
            ));
        } else {
    ...
    let code = code.ok_or_else(|| {
        "strata rule: error: the following arguments are required: code".to_owned()
    })?;
    Ok((color, code))
}
