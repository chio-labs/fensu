...
from fensu.rules.authoring.types import RuleKind, Threshold
...
def rule_context_lines() -> tuple[str, ...]:
    ...
    return (
        ...
        "top-level `fensu` package. The five public analysis zones are:",
        "",
...
def authoring_lookup_lines() -> tuple[str, ...]:
    ...
    return (
        ...
        "custom rules, then the public Fensu exports and type definitions from the project's "
        "active Python environment. This targets the installed Fensu version rather than "
        "remembered or generic API knowledge.",
        "",
        "It is acceptable to locate the active installation through `fensu.__file__` or the "
        ...
        "implementation only to diagnose a suspected Fensu defect.",
        "",
        "Only import authoring APIs from top-level `fensu`. Reading installed implementation for "
        "understanding does not make private `_helpers/` modules a supported dependency. Do not "
        "import from or couple custom rules to Fensu's private modules.",
        "",
    )
...
def rule_testing_lines(context: SkillGenerationContext) -> tuple[str, ...]:
    ...
    return (
        ...
        "",
        "Test approved custom rules through Fensu's real discovery and evaluation pipeline. "
