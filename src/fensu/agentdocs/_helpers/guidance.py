...
from fensu.agentdocs._helpers.role_examples import runtime_role_example_lines
...
from fensu.discovery.constants import HELPERS_DIRECTORY_NAME
...
def memory_retrieval_guidance_lines(config: Config) -> tuple[str, ...]:
    ...
    return (
        ...
        "",
        (
            "Use Fensu Memory for durable repository knowledge; keep transient reasoning and "
        ...
        "3. Use `fensu memory graph <document-or-pattern>` for relationship context third.",
        ...
        'fensu memory sql "SELECT identity, title, artifact_kind FROM '
        'memory.current_documents ORDER BY title"',
        'fensu memory sql "SELECT document_identity, heading_text, heading_path FROM '
        'memory.sections ORDER BY document_identity, ordinal"',
        'fensu memory sql "SELECT document_identity, heading_text, raw_markdown FROM '
        "memory.sections WHERE document_identity = 'task:...' ORDER BY ordinal\"",
        'fensu memory sql "SELECT identity, title FROM memory.blocked_tasks ORDER BY identity"',
...
def _container_guidance(*, config: Config, active_codes: frozenset[str]) -> tuple[str, ...]:
    ...
    if RoleCode.BANNED_GENERIC_PACKAGE_NAME in active_codes:
        lines.extend(
            (
                "Generic bucket names remain FFR204 concerns and do not receive a second "
