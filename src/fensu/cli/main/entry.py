"""Console-script entrypoint for the fensu CLI."""
...
from fensu.cli._helpers.lazy_commands import (
    run_check,
    run_init,
    run_map,
    run_memory,
...
from fensu.cli._helpers.stale_script import is_stale_console_script
from fensu.cli.types import CliCommand, CliOption
...
def main(argv: tuple[str, ...] | None = None) -> int:
    ...
    if argv is None and is_stale_console_script():
        sys.stderr.write(
            "The installed `fensu` command is a stale Python console script from an older "
            "fensu release. Reinstall fensu-cli, or use `python -m fensu`.\n"
        )
        return 2
    ...
    if args and args[0] == CliOption.VERSION:
        sys.stdout.write(f"fensu {_installed_version()}\n")
        return 0
    try:
