...
from fensu.analysis.classes.file_analysis import PythonFileAnalysis
from fensu.analysis.classes.lazy_syntax_artifacts import LazySyntaxArtifacts
...
from fensu.analysis.main.parse_native_program import parse_native_program
from fensu.analysis.main.parse_native_programs import parse_native_programs
...
def _raise_parse_error(*, scoped_file: ScopedFile, snapshot: SourceSnapshot) -> Never:
    ...
    try:
        ...
        message: str = (
            ...
            "interpreter running fensu. Run fensu under the target project's Python "
            "version or newer."
        )
        raise ParseError(
    ...
    message = (
        ...
        "interpreter but unsupported by the required native analyzer. Upgrade fensu "
