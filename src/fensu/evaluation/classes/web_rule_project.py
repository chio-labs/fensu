"""Requester-bound common project facts for hosted Web custom rules."""
...
class WebRuleProjectView:
    ...
    def tree(self) -> ProjectTree:
        """Return immutable authoritative Web discovery facts."""

        return self._tree

    def absolute_path(self, value: ProjectPath | str) -> Path:
        """Resolve one confined project path beneath the analyzed web target root."""
        ...
        if not absolute.is_relative_to(self._root):
            raise ProjectContextUnavailableError("Web project path escapes the analyzed project")
        return absolute
