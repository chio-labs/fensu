...
from fensu.public_api.constants import PUBLIC_ATTRIBUTES
from fensu.public_api.exceptions import UnknownPublicAttributeError
...
def load_public_attribute(name: str) -> object:
    ...
    if owner is None:
        raise UnknownPublicAttributeError(f"module 'fensu' has no attribute {name!r}")
