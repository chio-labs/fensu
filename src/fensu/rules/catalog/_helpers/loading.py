...
def build_catalogue_from_config(
    *, config: Config, repo_root: Path | None = None
    ...
    pack_rules: tuple[RuleSpec, ...] = tuple(
        rule
        for rule in (*FPDG_RULES, *FPRS_RULES, *FPTS_RULES, *FPSK_RULES)
        if rule.pack in config.rule_packs
    )
...
def _validate_rule_analyzers(*, rules: tuple[RuleSpec, ...]) -> None:
    for rule in rules:
        ...
        if rule.kind is RuleKind.CUSTOM and any(
            ...
            raise ConfigError(
                f"Custom rule {rule.code} declares unsupported analyzer applicability."
            )
...
def _synthetic_module_name(path: Path) -> str:
    ...
    return f"fensu._loaded_rules.{digest}"
...
def _repository_module_path(*, module: ModuleType, module_name: str, repo_root: Path) -> Path:
    ...
    if not path.is_file() or not path.is_relative_to(repo_root):
        raise ConfigError(
            ...
            "FFR707 diagnostics and cache entries can be owned by its source."
        )
    return path
...
def _configured_module_name(
    *, rule: RuleSpec, source_path: Path, config: Config, repo_root: Path
    ...
    if not rule.check.__module__.startswith("fensu._loaded_rules."):
