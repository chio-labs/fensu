...
from fensu.rules.tests.types import FftCode, TestPathName, TestScope, TestSymbol
...
_test_function_rule_codes: frozenset[FftCode] = frozenset(
    {
...
_layout_rule_codes: frozenset[FftCode] = frozenset(
    {
        FftCode.TEST_LAYOUT,
        FftCode.TEST_SCOPE,
...
_local_types_rule_codes: frozenset[FftCode] = frozenset(
    {
...
def test_faults(*, ctx: RuleContext, code: FftCode) -> list[Fault]:
    """Collect faults for a single tests-family rule."""

    if ctx.scope() is not ScopeName.TEST:
        return []
    if code == FftCode.NO_IF_IN_TESTS:
    ...
    if code in {FftCode.TEST_TYPES_DESCRIPTION, FftCode.TEST_TYPES_EXPECTED_FIELD}:
        return []
...
def _layout_faults(*, ctx: RuleContext, code: FftCode) -> list[Fault]:
...
def _layout_issue(*, ctx: RuleContext) -> _LayoutIssue | None:
    ...
    return _LayoutIssue(
        code=FftCode.TEST_MIRRORED_ROOT,
...
def _tooling_layout_issue(
    *, ctx: RuleContext, mirrored_parts: tuple[str, ...], source_root: Path
    ...
    if len(mirrored_parts) <= len(source_parts):
        return _LayoutIssue(
            code=FftCode.SCRIPTS_MIRROR_DEPTH,
    ...
    if not ctx.project.exists(requester=ctx.path, path=area_path):
        return _LayoutIssue(
            code=FftCode.SCRIPTS_AREA_EXISTS,
...
def _unmatched_runtime_layout_issue(
    *, ctx: RuleContext, mirrored_parts: tuple[str, ...], runtime_roots: tuple[Path, ...]
    ...
    for root in runtime_roots:
        ...
        return _LayoutIssue(
            code=FftCode.SRC_PACKAGE_EXISTS,
...
def _selected_path_faults(
    *, ctx: RuleContext, code: FftCode, actual_code: FftCode, message: str
) -> list[Fault]:
    if code != actual_code:
        return []
...
def _path_fault(*, ctx: RuleContext, code: FftCode, message: str) -> Fault:
    del code
...
def _test_types_faults(*, ctx: RuleContext, code: FftCode) -> list[Fault]:
    faults: list[Fault] = []
    for fact in ctx.facts.dataclasses():
        if (
            code == FftCode.TEST_TYPES_DESCRIPTION
            and TestSymbol.DESCRIPTION not in fact.field_names
        ):
            faults.append(ctx.fault_at(location=fact.location))
        if code == FftCode.TEST_TYPES_EXPECTED_FIELD and not any(
...
def _test_file_faults(*, ctx: RuleContext, code: FftCode) -> list[Fault]:
    local_test_types: _LocalTestTypes | None = None
    inspect_dataclasses: bool = code != FftCode.LOCAL_TEST_TYPES_IMPORT
    test_module: bool = _is_test_module(ctx.path)
    if (code in _local_types_rule_codes or code in _test_function_rule_codes) and test_module:
    ...
    faults: list[Fault] = []
    if (
        code == FftCode.LOCAL_TEST_TYPES_FILE
        and test_module
    ...
    if code == FftCode.TEST_FILE_NAME and test_module and not ctx.path.name.startswith("test_"):
        faults.append(ctx.path_fault())
    ...
    if code in _test_function_rule_codes:
        locations_by_code: dict[FftCode, tuple[SourceLocation, ...]] = ctx._memoize(
            key="tests.function_issue_locations",
            operation=lambda: _test_function_issue_locations(
                ctx=ctx,
                module_context=module_context,
            ),
        )
        faults.extend(ctx.fault_at(location=location) for location in locations_by_code[code])
    return faults


def _module_shape_faults(*, ctx: RuleContext, code: FftCode) -> list[Fault]:
    if not _is_test_module(ctx.path):
        return []
    facts: PytestModuleFacts = ctx.facts.test_module()
    if code == FftCode.NO_TOP_LEVEL_HELPERS:
        return [ctx.fault_at(location=location) for location in facts.top_level_helper_locations]
    if code == FftCode.PRIVATE_CONSTANT_ORDER:
...
def _local_import_faults(
    *,
    ...
    if code != FftCode.LOCAL_TEST_TYPES_IMPORT or local_test_types is None:
        return []
    faults: list[Fault] = []
...
def _test_function_issue_locations(
    *,
    ctx: RuleContext,
    module_context: _TestModuleContext,
) -> dict[FftCode, tuple[SourceLocation, ...]]:
    grouped: dict[FftCode, list[SourceLocation]] = {code: [] for code in _test_function_rule_codes}
    for fact in ctx.facts.test_functions():
        if not _test_name_pattern.match(fact.name):
            grouped[FftCode.TEST_FUNCTION_NAME].append(fact.location)
        parametrize: ParametrizeFact | None = fact.parametrize
        if parametrize is None:
            grouped[FftCode.DATACLASS_PARAMETRIZE].append(fact.location)
            continue
        test_case_present: bool = TestSymbol.TEST_CASE in fact.parameter_names
        if not test_case_present:
            grouped[FftCode.ACCEPTS_TEST_CASE].append(fact.location)
        if (
            not test_case_present
            or fact.test_case_annotation_name not in module_context.test_case_annotation_names
        ):
            grouped[FftCode.TEST_CASE_ANNOTATION].append(fact.location)
        grouped = _append_parametrize_issue_locations(
            function=fact,
            decorator=parametrize,
            module_context=module_context,
            grouped=grouped,
        )
        if not fact.references_expected_field:
            grouped[FftCode.EXPECTED_FIELD_ASSERTION].append(fact.location)
    return {code: tuple(locations) for code, locations in grouped.items()}


def _append_parametrize_issue_locations(
    *,
    function: PytestFunctionFact,
    decorator: ParametrizeFact,
    module_context: _TestModuleContext,
    grouped: dict[FftCode, list[SourceLocation]],
) -> dict[FftCode, list[SourceLocation]]:
    if decorator.argument_count < _minimum_parametrize_arguments:
        grouped[FftCode.PARAMETRIZE_ARGUMENTS].append(function.location)
        return grouped
    if decorator.parameter_name != TestSymbol.TEST_CASE:
        grouped[FftCode.PARAMETRIZE_TEST_CASE].append(function.location)
    if not decorator.ids_present:
        grouped[FftCode.PARAMETRIZE_IDS].append(function.location)
    if not decorator.values_is_sequence and not decorator.values_is_comprehension:
        grouped[FftCode.INLINE_PARAMETRIZE_VALUES].append(function.location)
        return grouped
    if decorator.values_is_comprehension:
        if not _is_local_constructor(
            constructor_name=decorator.cases[0].constructor_name, context=module_context
        ):
            grouped[FftCode.LOCAL_TEST_CASE_CONSTRUCTORS].append(decorator.cases[0].location)
        if not decorator.description_lambda_ids:
            grouped[FftCode.DESCRIPTION_LAMBDA_IDS].append(function.location)
        return grouped
    if decorator.values_empty:
        grouped[FftCode.NONEMPTY_PARAMETRIZE_VALUES].append(function.location)
    for case in decorator.cases:
        if case.dictionary:
            grouped[FftCode.NO_DICT_TEST_CASES].append(case.location)
        elif not _is_local_constructor(
            constructor_name=case.constructor_name, context=module_context
        ):
            grouped[FftCode.LOCAL_TEST_CASE_CONSTRUCTORS].append(case.location)
    if not decorator.description_lambda_ids:
        grouped[FftCode.DESCRIPTION_LAMBDA_IDS].append(function.location)
    return grouped
