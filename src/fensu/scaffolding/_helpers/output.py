"""Transcript rendering through Fensu's semantic CLI style facade."""
...
from fensu.scaffolding.models import DriftSummary, InitPlan, PathCandidate
...
def write_classification(
    *, stdout: TextIO, style: CliStyle, plan: InitPlan, python_file_count: int
    ...
    stdout.write(f"\n    Enabling the full Fensu ruleset: {style.value('FF')}\n")
    stdout.write(f"    Wrote {style.success('fensu.toml')}\n")
...
def write_next(*, stdout: TextIO, style: CliStyle) -> None:
    ...
    stdout.write("\n")
    stdout.write(f"    {style.value('fensu check')}            {style.hint('run anytime')}\n")
    stdout.write(
        f"    {style.value('fensu rule FFA001')}      "
