...
from fensu.scaffolding.exceptions import InitError, InitRefusalError
...
from fensu.scaffolding.types import CandidateProvenance, InteractionDecision
...
def preflight_existing_config(*, repository: Path) -> Path | None:
    ...
    if source is not None:
        raise InitRefusalError(f"Fensu configuration already exists: {source.path}")
    return None
...
def find_local_config(*, repository: Path) -> Path | None:
    ...
    local_content: bytes | None = _capture_local_file(path=local_path, label="Fensu configuration")
    ...
    if pyproject_content is not None and _has_tool_fensu(
...
def _has_tool_fensu(*, path: Path, content: bytes) -> bool:
    try:
    ...
    return isinstance(tool, dict) and isinstance(tool.get("fensu"), dict)
