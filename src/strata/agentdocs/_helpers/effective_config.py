...
def effective_config_lines(context: SkillGenerationContext) -> tuple[str, ...]:
    ...
    lines: list[str] = [
        ...
        "authorization. Run `strata check --warn` after substantial changes when practical, and "
...
def warning_policy_lines(context: SkillGenerationContext) -> tuple[str, ...]:
    ...
    return (
        "## Warning Policy",
        "",
        "`strata check` evaluates blocking rules only. `strata check --warn` additionally "
