...
def _main_example(*, package: str) -> tuple[str, ...]:
    return (
        ...
        "",
        (
            "Expose exactly one public entry function and keep phase work in helpers. Use up to "
            "two private functions only when entry-specific glue is genuinely needed:"
        ),
        "",
        "```python",
        f"from {package}.billing.invoices.helpers.loading import load_invoice",
        f"from {package}.billing.invoices.helpers.normalization import normalize_invoice",
        f"from {package}.billing.invoices.models import Invoice",
        "",
