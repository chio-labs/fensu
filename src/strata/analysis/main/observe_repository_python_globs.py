"""Observe ordered `*.py` repository globs through one optional native batch."""
...
def observe_repository_python_globs(
    *,
    repo_root: Path,
    queries: tuple[tuple[str, bool], ...],
) -> tuple[RepositoryGlobAnswer | None, ...]:
    ...
    try:
        ...
        rows: list[tuple[str, list[str]] | None] = strata_facts.observe_repository_python_globs(
            repo_root,
            list(queries),
        )
    ...
    if len(rows) != len(queries):
        return unavailable
    answers: list[RepositoryGlobAnswer | None] = []
    for row in rows:
        if (
            row is None
            ...
            or not all(isinstance(path, str) for path in row[1])
        ):
            answers.append(None)
            continue
        answers.append(
            RepositoryGlobAnswer(
                dependency_path=row[0],
                answer=tuple(row[1]),
            )
        )
