...
@pytest.mark.parametrize(
    ...
    [
        ...
        ConfigurableLayoutCliTestCase(
            description="installed check aggregates every configured target",
            config=_TARGET_CONFIG,
            files=_TARGET_FILES,
            working_directory=".",
            ...
            expected_stdout_fragments=(
                "XNT001",
                "frontend/src/custom/module.py",
                "Found 1 fault",
            ),
            expected_stderr_fragments=(),
        ),
        ConfigurableLayoutCliTestCase(
            description="installed all-target custom check emits one aggregate cache status",
            config=_TARGET_CONFIG,
            files=_TARGET_FILES,
            working_directory=".",
            argv=("check", "--no-color", "--cache", "--cache-stats"),
            expected_exit_code=1,
            expected_stdout_fragments=("frontend/src/custom/module.py", "Found 1 fault"),
            expected_stderr_fragments=("Cache: hits=",),
        ),
    ],
