...
from fensu.cache.mapping.models import CachedCallMap, FileDeclarations, MapManifest
from fensu.cache.storage.classes.cache_store import CacheStore
from fensu.cache.storage.models import CacheMutationOutcome, CacheRead, CacheRecord
from fensu.cache.storage.types import CacheMutator
...
def write_invalid_cache_config(root: Path) -> None:
    ...
    (root / "fensu.toml").write_text("roots = [\n", encoding="utf-8")
...
def fail_mapping_identity(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "fensu.cache.mapping._helpers.evaluation.build_mapping_identity",
        fail,
    )
