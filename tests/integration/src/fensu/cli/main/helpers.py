...
class ConcurrentExistingSkillWriter:
    ...
    def _race(self) -> None:
        replacement: Path = self._target_path.with_name("concurrent-user-skill")
...
class ConcurrentLegacyRenamer:
    ...
    def _race(self) -> None:
        self._raced = True
        replacement: Path = self._legacy_path.with_name("concurrent-user-legacy")
        replacement.write_text(self._user_content, encoding="utf-8")
...
def write_init_existing_config(*, root: Path, source: str) -> Path:
    ...
    writer: Callable[[], Path] = {
        "fensu.toml": write_fensu_config,
        "tool.fensu": write_pyproject_config,
    }[source]
    return writer()
...
def assert_init_execution_files(
    *,
    ...
    def assert_success() -> None:
        ...
        assert (root / "fensu.toml").read_text(encoding="utf-8") == expected_config
...
def prepare_init_refusal_project(*, root: Path, source: str) -> tuple[Path, tuple[str, ...]]:
    ...
    }[source in {"fensu.toml", "tool.fensu"}]
    write_parent: Callable[[], object] = {
        False: lambda: None,
        True: lambda: write_init_existing_config(root=root, source="fensu.toml"),
    }[source == "parent"]
    _ = write_local()
    _ = write_parent()
...
def write_broken_fensu_symlink(*, root: Path, outside_target: Path) -> None:
    """Write a local config symlink whose outside target does not exist."""

    (root / "fensu.toml").symlink_to(outside_target)
...
def prepare_unsafe_local_config_target(*, root: Path, target_kind: str) -> Path:
    ...
    name: str = {False: "fensu.toml", True: "pyproject.toml"}[target_kind.startswith("pyproject")]
    path: Path = root / name

    def create_symlink() -> None:
        outside: Path = root.parent / f"{root.name}-{name}"
        outside.write_text('[tool.fensu]\nroots = ["src/pkg"]\n', encoding="utf-8")
...
def write_cli_fixture_project(
    *, root: Path, rule_code: str, include_core_rules: bool = False
    ...
    ignored_rules: str = {False: "", True: 'ignore = ["FFH002"]\n'}[include_core_rules]
    config: str = (
...
def generated_skill_text(*, root: Path, body: str) -> str:
    ...
    owner: str = hashlib.sha256(
        (root / "fensu.toml").resolve().as_posix().encode("utf-8")
    ).hexdigest()
    marker: str = json.dumps(
        {
            "content_fingerprint": "content",
            "identity": "fensu-fixture",
    ...
    generated: str = "<!-- generated-by: fensu skills update -->"
    return f"{generated}\n<!-- fensu-skill-owner: {marker} -->\n{body}"
...
def mutate_skill_freshness_state(*, root: Path, state: str) -> None:
    ...
    path: Path = root / ".agents/skills/fensu-fixture/SKILL.md"
    mutations: dict[str, Callable[[], object]] = {
        "current": lambda: None,
        "missing": path.unlink,
        "stale": lambda: (root / "fensu.toml").write_text(
            (root / "fensu.toml").read_text(encoding="utf-8") + "\n[cache]\nenabled = false\n",
            encoding="utf-8",
        ),
...
def write_cli_no_fault_project(root: Path) -> None:
    ...
    (root / "fensu.toml").write_text('roots = ["src/pkg"]\n', encoding="utf-8")
...
def write_cli_map_project(
    *,
    ...
    write_configuration: Callable[[], object] = {
        False: lambda: None,
        True: lambda: (root / "fensu.toml").write_text(
...
def write_cli_method_map_project(root: Path) -> None:
    ...
    package.mkdir(parents=True)
    (root / "fensu.toml").write_text('roots = ["src/methods"]\n', encoding="utf-8")
