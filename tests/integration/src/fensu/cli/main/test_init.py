"""Integration tests for repository-aware `fensu init`."""
...
from fensu.cli.main._init import run_init
...
from fensu.reporting.constants import ANSI_BOLD_RED, ANSI_ORANGE
from fensu.scaffolding._helpers.execution import build_rendered_config, render_config
from fensu.scaffolding.models import InitPlan
...
@pytest.mark.parametrize(
    ...
    [
        ...
        InitExecutionTestCase(
            ...
            expected_exit_code=0,
            expected_config=('roots = ["src/acme"]\ntests = ["tests"]\nselect = ["FF"]\n'),
            expected_output_fragments=("Wrote fensu.toml",),
    ...
    [
        InitOptionTestCase(
            ...
            expected_skill_paths=(
                ".opencode/skills/fensu-acme/SKILL.md",
                ".claude/skills/fensu-acme/SKILL.md",
                ".agents/skills/fensu-acme/SKILL.md",
            ),
        ),
        InitOptionTestCase(
...
def test_given_explicit_scope_adoption_and_skill_options_when_initializing_then_honors_all_flags(
    ...
    assert ("Run fensu skills" in stdout.getvalue()) is (not test_case.expected_skill_paths)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        InitRefusalTestCase(
            ...
            source="parent",
            scripted_input="",
            expected_error_fragment="Fensu configuration already exists:",
        ),
    ],
    ids=lambda case: case.description,
)
def test_given_decline_or_existing_configuration_when_initializing_then_refuses_without_write(
    ...
    assert (repository / "fensu.toml").exists() is (test_case.source == "fensu.toml")
...
@pytest.mark.parametrize(
    ...
    [
        InitRerunTestCase(
            description="local fensu toml ignores valid different init flags",
            source="fensu.toml",
            argv=("--name", "ignored"),
            expected_relative_config="fensu.toml",
            expected_exit_code=0,
        ),
        InitRerunTestCase(
            description="local tool fensu table ignores different init flags",
            source="tool.fensu",
...
def test_given_local_configuration_when_rerunning_init_then_succeeds_before_options_or_tty(
    ...
    assert stdout.getvalue() == (
        f"Fensu configuration already exists: {expected_path} (nothing to do)\n"
    )
    assert stderr.getvalue() == ""
...
@pytest.mark.parametrize(
    ...
    [
        InitExecutionTestCase(
            ...
            expected_exit_code=0,
            expected_config=None,
            expected_output_fragments=("Run fensu skills when you are ready.",),
        )
    ],
    ids=lambda case: case.description,
)
...
def test_given_monorepo_project_when_initializing_then_installs_at_git_root_with_prefixed_paths(
    ...
    assert all(fragment in stdout.getvalue() for fragment in test_case.expected_output_fragments)
    assert all((tmp_path / path).is_file() for path in test_case.expected_created_paths)
    assert 'Configuration source: "backend/fensu.toml"' in content
    ...
    [
        ...
        InitPresentationTestCase(
            ...
            expected_output_fragments=(
                ...
                f"{ANSI_ORANGE}FFA\033[0m",
                f"{ANSI_BOLD_RED}1 fault\033[0m",
            ),
            expected_absent_fragments=(f"{ANSI_ORANGE}Detecting", f"{ANSI_ORANGE}fensu.toml"),
        ),
        InitPresentationTestCase(
            ...
            no_color=True,
            include_fault=True,
            expected_output_fragments=("--> Detecting project layout", "FFA  annotations"),
            expected_absent_fragments=("\033[",),
        ),
        InitPresentationTestCase(
...
@pytest.mark.parametrize(
    ...
    [
        InitSymlinkRefusalTestCase(
            description="broken local fensu config symlink is refused without touching outside target",
            expected_exit_code=2,
            expected_error_fragment="Fensu configuration path is a symlink:",
            expected_stdout="",
        )
    ],
    ids=lambda case: case.description,
...
def test_given_broken_local_config_symlink_when_initializing_then_refuses_without_outside_write(
    ...
    write_init_hatch_project(root=tmp_path)
    write_broken_fensu_symlink(root=tmp_path, outside_target=outside_target)
    stdout: TerminalBuffer = TerminalBuffer(is_terminal=False)
    stderr: TerminalBuffer = TerminalBuffer(is_terminal=False)
    ...
    assert (tmp_path / "fensu.toml").is_symlink()
...
@pytest.mark.parametrize(
    ...
    [
        InitLocalTargetTestCase(
            description="local fensu config directory is refused",
            target_kind="fensu-directory",
            expected_error_fragment="Fensu configuration path is not a regular file:",
            expected_exit_code=2,
        ),
        InitLocalTargetTestCase(
...
def test_given_unsafe_local_config_target_when_initializing_then_refuses_without_publication(
    ...
    assert not (tmp_path / "fensu.toml").is_file()
...
def test_given_eof_before_first_prompt_response_when_initializing_then_exits_without_write(
    ...
    assert (tmp_path / "fensu.toml").exists() is test_case.expected_config_written
    assert project_file_snapshot(tmp_path) == before
