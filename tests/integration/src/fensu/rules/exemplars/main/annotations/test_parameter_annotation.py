...
@pytest.mark.parametrize(
    ...
    [
        ...
        NativeCustomRuleParityTestCase(
            description="FFL104 uses the domain after a configured structural group",
            native_code="FFL104",
            source="from example.sources.customers.main._load import load\n",
            expected_fault_count=1,
            path="src/example/sources/orders/main/run.py",
            files=(
                RuleFile(
                    path="src/example/sources/customers/main/_load.py",
        ...
        NativeCustomRuleParityTestCase(
            description="FFR306 accepts a structural group above a leaf domain",
            native_code="FFR306",
            source="",
            expected_fault_count=0,
        ...
        NativeCustomRuleParityTestCase(
            description="FFR309 recognizes grouped domain and subdomain leaves",
            native_code="FFR309",
            source="def run() -> None:\n    pass\n",
            expected_fault_count=0,
...
def test_given_shared_fixture_when_evaluating_native_and_custom_rules_then_faults_match(
    ...
    # The FFL102 exemplar consumes the public graph rather than reproducing native probes.
    dependency_pair: tuple[object, object] = {
        "FFL102": ((), ()),
    }.get(
        test_case.native_code,
        (native_result.dependencies, custom_result.dependencies),
    )
    assert dependency_pair[0] == dependency_pair[1]
