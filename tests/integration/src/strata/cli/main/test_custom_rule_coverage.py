...
_DYNAMIC_CASES: str = (
    "from strata import evaluate_rule\n"
    "from rules.custom_rule import first_rule\n\n"
...
def test_given_configured_custom_rules_when_checking_coverage_then_reports_source_owned_results(
    ...
    assert stdout.getvalue().count("SFR707  ") == test_case.expected_fault_count
...
def test_given_cached_coverage_when_test_namespace_changes_then_invalidates_rule_source_owner(
    ...
    (tmp_path / "strata.toml").write_text(
        (tmp_path / "strata.toml").read_text(encoding="utf-8") + "\n[cache]\nenabled = true\n",
        encoding="utf-8",
    )
    monkeypatch.chdir(tmp_path)
    ...
    assert "SFR707" not in cleanup_stdout.getvalue()
...
def test_given_external_custom_rule_module_when_checking_then_returns_actionable_config_error(
    ...
    assert "SFR707 diagnostics and cache entries" in stderr.getvalue()
...
def test_given_warn_only_coverage_when_running_uncached_then_respects_warning_mode_and_tier(
    ...
    assert "SFR707" not in plain_stdout.getvalue()
    ...
    assert warning_stdout.getvalue().count("SFR707  ") == 1
...
def test_given_warn_only_coverage_when_running_cached_then_replays_warning_source_record(
    ...
    assert "SFR707" not in plain_cold_stdout.getvalue()
    ...
    assert warning_cold_stdout.getvalue().count("SFR707  ") == 1
