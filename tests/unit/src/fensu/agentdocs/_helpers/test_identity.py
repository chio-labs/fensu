...
from fensu.config.models import Config, ConfigSource, SkillsConfig
from fensu.config.types import ConfigSourceKind
from tests.unit.src.fensu.agentdocs._helpers._test_types import (
    GitMarkerTestCase,
    InstallRootTestCase,
    SkillNormalizationTestCase,
)
from tests.unit.src.fensu.agentdocs._helpers.helpers import (
    write_git_marker,
...
@pytest.mark.parametrize(
    ...
    [
        SkillNormalizationTestCase(
            ...
            expected_identity="fensu-configured-api",
        )
    ],
    ids=lambda case: case.description,
)
def test_given_configured_identity_when_resolving_then_it_has_highest_precedence(
    ...
    source: ConfigSource = ConfigSource(
        path=project / "fensu.toml", kind=ConfigSourceKind.FENSU_TOML
    )
...
@pytest.mark.parametrize(
    ...
    [
        SkillNormalizationTestCase(
            ...
            expected_identity="fensu-distribution-name",
        )
    ],
    ids=lambda case: case.description,
)
...
@pytest.mark.parametrize(
    ...
    [
        SkillNormalizationTestCase(
            ...
            value="backend_api",
            expected_identity="fensu-backend-api",
        )
    ],
    ids=lambda case: case.description,
)
...
@pytest.mark.parametrize(
    ...
    [
        SkillNormalizationTestCase(
            description="Fensu repository defaults from its distribution metadata",
            value="fensu",
            expected_identity="fensu-fensu",
        )
    ],
    ids=lambda case: case.description,
)
def test_given_fensu_repository_when_resolving_then_uses_fensu_distribution_identity(
    test_case: SkillNormalizationTestCase,
) -> None:
    repository: Path = Path(__file__).resolve().parents[6]
    source: ConfigSource = ConfigSource(
        path=repository / "fensu.toml",
        kind=ConfigSourceKind.FENSU_TOML,
    )

    result: str = resolve_skill_name(
        config=Config(roots=("src/fensu",)),
        source=source,
