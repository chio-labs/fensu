...
@pytest.mark.parametrize(
    ...
    [
        ConfigSourceLoadTestCase(
            description="dedicated fensu toml loads top-level config",
            fensu_toml='roots = ["src/dedicated"]\nselect = ["FFL"]\n',
            pyproject_toml="",
...
def test_given_fensu_toml_when_loading_then_returns_expected_config(
    tmp_path: Path,
    test_case: ConfigSourceLoadTestCase,
) -> None:
    write_fensu_toml(root=tmp_path, contents=test_case.fensu_toml or "")
...
@pytest.mark.parametrize(
    ...
    [
        ConfigSourceLoadTestCase(
            description="pyproject tool fensu loads nested config",
            fensu_toml="",
            pyproject_toml='[tool.fensu]\nroots = ["src/pyproject"]\nselect = ["FFR"]\n',
...
def test_given_pyproject_tool_fensu_when_loading_then_returns_expected_config(
    tmp_path: Path,
    test_case: ConfigSourceLoadTestCase,
) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ConfigDiscoveryTestCase(
            ...
            parent_fensu_toml='roots = ["src/pkg"]\n',
            expected_roots=("src/pkg",),
        ),
        ConfigDiscoveryTestCase(
            description="pyproject without tool fensu is skipped while walking upward",
            start_relative_path="workspace/pkg/module.py",
            child_pyproject_toml='[project]\nname = "not-fensu"\n',
            parent_fensu_toml='roots = ["workspace/pkg"]\n',
...
def test_given_nested_start_when_loading_then_discovers_parent_config(
    ...
    write_fensu_toml(root=tmp_path, contents=test_case.parent_fensu_toml or "")
...
@pytest.mark.parametrize(
    ...
    [
        InvalidConfigSourceTestCase(
            ...
            expected_error_fragment="No fensu config found",
        ),
    ],
    ids=lambda case: case.description,
)
