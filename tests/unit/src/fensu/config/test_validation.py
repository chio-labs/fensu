...
@pytest.mark.parametrize(
    ...
    [
        ...
        InvalidConfigTestCase(
            description="nested roots are rejected parent first and name both paths",
            config_text='roots = ["src", "src/pkg"]\n',
            expected_error_type=ConfigError,
            expected_error_fragment="nested paths: 'src' contains 'src/pkg'",
        ),
        InvalidConfigTestCase(
            description="nested roots are rejected child first and name the outer path first",
            config_text='roots = ["src/pkg", "src"]\n',
            expected_error_type=ConfigError,
            expected_error_fragment="nested paths: 'src' contains 'src/pkg'",
        ),
        InvalidConfigTestCase(
            description="duplicate roots are reported as duplicates rather than nesting",
            config_text='roots = ["src/pkg", "src/pkg"]\n',
            expected_error_type=ConfigError,
            expected_error_fragment="duplicate paths: 'src/pkg'",
        ),
    ],
    ids=lambda case: case.description,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        InvalidConfigTestCase(
            ...
            config_text='roots = ["src/pkg"]\nselect = ["FFL0001"]\n',
            expected_error_type=ConfigValidationError,
            expected_error_fragment="FFL0001",
        ),
        InvalidConfigTestCase(
...
@pytest.mark.parametrize(
    ...
    [
        RuleSelectorConfigTestCase(
            ...
            config_text=(
                'roots = ["src/pkg"]\nselect = ["FF", "FFR", "FFR3", "FFZ001", "XDB"]\n'
                'warn = ["FFS102", "XAD"]\n'
                'ignore = ["FFR30", "XDB001"]\n'
            ),
            expected_select=("FF", "FFR", "FFR3", "FFZ001", "XDB"),
            expected_warn=("FFS102", "XAD"),
            expected_ignore=("FFR30", "XDB001"),
        )
    ],
    ids=lambda case: case.description,
)
