...
def test_given_module_when_reading_ast_helpers_then_returns_expected_facts(
    ...
    indexes: SyntaxIndexes = index_module_nodes(module=module)

    assert len(indexes.node_index[ast.Call]) == test_case.expected_call_count
...
def test_given_module_when_building_indexes_then_visits_each_node_once(
    ...
    assert sum(len(nodes) for nodes in indexes.node_index.values()) == test_case.expected_node_count
    assert len(indexes.parent_by_node) == test_case.expected_parent_count
...
def test_given_core_rules_when_inspecting_walks_then_only_holistic_analysis_walks_module(
    ...
    direct_walk_paths: tuple[str, ...] = direct_module_walk_paths(root=Path("src/fensu/rules"))
...
def test_given_core_rules_when_inspecting_context_then_no_private_analysis_zones_are_used(
    ...
    private_zone_paths: tuple[str, ...] = private_context_zone_paths(root=Path("src/fensu/rules"))
