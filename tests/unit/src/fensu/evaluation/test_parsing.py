...
from fensu.evaluation._helpers import parsing
...
from tests.unit.src.fensu.discovery.helpers import make_config
...
from tests.unit.src.fensu.evaluation.helpers import direct_ast_parse_paths, write_sources
...
@pytest.mark.parametrize(
    ...
    [
        ParseErrorTestCase(
            ...
            source="def broken(:\n    pass\n",
            expected_error_fragment="Run fensu under the target project's Python version or newer",
            expected_line=1,
            expected_column=12,
        )
    ],
...
def test_given_source_snapshot_when_evaluation_parses_then_delegates_to_analysis_factory(
    ...
    direct_parse_paths: tuple[str, ...] = direct_ast_parse_paths(root=Path("src/fensu/evaluation"))
