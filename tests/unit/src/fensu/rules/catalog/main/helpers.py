...
check.__fensu_rule_spec__ = RuleSpec(
    code={rule_code!r},
...
def write_importing_custom_rule_package(*, root: Path, rule_code: str) -> Path:
    ...
    package: Path = root / "scripts/fensu"
...
from scripts.fensu._helpers.names import RULE_MESSAGE
...
def make_core_rule(*, code: str, family: Family, enabled_by_default: bool = True) -> RuleSpec:
    ...
    return RuleSpec(
        ...
        message=code,
        check=check,
        kind={False: RuleKind.CUSTOM, True: RuleKind.CORE}[code.startswith("FF")],
