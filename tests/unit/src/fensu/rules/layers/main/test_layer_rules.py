...
from tests.unit.src.fensu.rules.layers.main import helpers as layer_test_helpers
from tests.unit.src.fensu.rules.layers.main._test_types import (
    LayerRuleTestCase,
    NativeLayerRegistryTestCase,
    ToolingImportRuleTestCase,
)
from tests.unit.src.fensu.rules.layers.main.helpers import (
    evaluate_layer_rule,
...
@pytest.mark.parametrize(
    ...
    [
        LayerRuleTestCase(
            description="native FFL001 bypasses its Python core callback",
            rule_code="FFL001",
        ...
        LayerRuleTestCase(
            description="native FFL002 bypasses its Python core callback",
            rule_code="FFL002",
        ...
        LayerRuleTestCase(
            description="native FFL101 bypasses its Python core callback",
            rule_code="FFL101",
            files=(
        ...
        LayerRuleTestCase(
            description="native FFL102 bypasses its Python core callback",
            rule_code="FFL102",
        ...
        LayerRuleTestCase(
            description="native FFL104 bypasses its Python core callback",
            rule_code="FFL104",
            files=(
        ...
        LayerRuleTestCase(
            description="native FFL103 bypasses its Python core callback",
            rule_code="FFL103",
        ...
        LayerRuleTestCase(
            description="native FFL105 bypasses its Python core callback",
            rule_code="FFL105",
        ...
        LayerRuleTestCase(
            description="native FFL110 bypasses its Python core callback",
            rule_code="FFL110",
            files=(
                (
                    "src/pkg/domain/alpha/main/run.py",
        ...
        LayerRuleTestCase(
            description="native FFL301 bypasses its Python core callback",
            rule_code="FFL301",
            files=(("src/pkg/domain/alpha/main/run.py", "from scripts.tools import helper\n"),),
            tooling=("scripts",),
            expected_codes=("FFL301",),
            expected_lines=(1,),
        ),
    ],
    ids=lambda case: case.description,
...
def test_given_registered_native_layer_rule_when_evaluating_then_skips_python_callback(
    ...
    rules_by_code: dict[str, RuleSpec] = {rule.code: rule for rule in layer_test_helpers.FFL_RULES}
    monkeypatch.setattr(
        layer_test_helpers,
        "FFL_RULES",
...
def test_given_layer_registry_when_selecting_native_rules_then_all_core_layers_are_native(
    ...
    layer_codes: set[str] = {rule.code for rule in layer_test_helpers.FFL_RULES}
...
def test_given_runtime_imports_tooling_when_checking_layers_then_flags_only_runtime_import(
    ...
    result: EvaluationResult = evaluate_layer_rule(
        rule_code="FFL301", config=config, monkeypatch=monkeypatch, repo_root=tmp_path
    )
