"""Tests for the roles rule catalogue."""

from __future__ import annotations

import pytest

from fensu.rules.authoring.models import RuleSpec
from fensu.rules.authoring.types import ExecutionOwner
from fensu.rules.roles.constants import FFR_RULES
from fensu.rules.roles.types import RoleCode
from tests.unit.src.fensu.rules.roles._test_types import FfrCatalogueTestCase


@pytest.mark.parametrize(
    "test_case",
    [
        FfrCatalogueTestCase(
            description="roles rule catalogue matches implemented role codes",
            expected_codes=tuple(code.value for code in RoleCode),
            expected_unique_count=len(RoleCode),
            expected_sfr204_message="runtime package directories must identify an owner",
            expected_sfr301_owner=ExecutionOwner.PACKAGE,
            expected_sfr302_owner=ExecutionOwner.PACKAGE,
            expected_sfr303_slug="helpers-reserved-role-filenames",
            expected_sfr303_message=("_helpers/ packages must not contain reserved role filenames"),
            expected_sfr303_remediation=(
                "Rename the helper module after its specific operation, or move role-owned "
                "declarations to the corresponding sibling models, types, constants, or "
                "exceptions role."
            ),
            expected_sfr306_symbolic_name="TOP_LEVEL_DOMAIN_SHAPE",
            expected_sfr306_slug="top-level-domain-shape",
            expected_sfr306_message=(
                "top-level domains must be either role leaves or subdomain branches"
            ),
            expected_sfr306_remediation=(
                "Keep direct role content in a leaf domain, or move it into a named subdomain "
                "when the domain contains subdomains."
            ),
            expected_sfr306_owner=ExecutionOwner.DOMAIN,
            expected_sfr307_message="top-level domains must not contain ad hoc direct modules",
            expected_sfr307_remediation=(
                "Move the module under a direct role boundary or into an owning named subdomain."
            ),
            expected_sfr308_slug="shared-domain-prefix",
            expected_sfr308_message=(
                "sibling domains must not encode one parent domain through a shared name prefix"
            ),
            expected_sfr308_remediation=(
                "Create one parent domain from the shared prefix and move each remaining suffix "
                "beneath it as a named subdomain."
            ),
            expected_sfr308_owner=ExecutionOwner.SCOPE,
            expected_sfr309_slug="leaf-main-boundary",
            expected_sfr309_message=(
                "leaf runtime domains and subdomains must expose meaningful behavior through main/"
            ),
            expected_sfr309_remediation=(
                "Add a focused main/ entry module only when the leaf owns behavior; otherwise "
                "move passive declarations into the closest domain or subdomain whose main/ "
                "behavior owns and uses them."
            ),
            expected_sfr309_owner=ExecutionOwner.LEAF,
            expected_sfr706_slug="descriptive-rule-module-names",
            expected_sfr706_message=(
                "rule module filenames must describe their policy rather than repeat one rule code"
            ),
            expected_sfr706_remediation=(
                "Rename the module after the policy or rule family it implements, using a name "
                "such as conditional_test_flow.py instead of fft104.py."
            ),
        )
    ],
    ids=lambda case: case.description,
)
def test_given_roles_rule_catalogue_when_reading_codes_then_matches_role_code_enum(
    test_case: FfrCatalogueTestCase,
) -> None:
    codes: tuple[str, ...] = tuple(rule.code for rule in FFR_RULES)

    assert codes == test_case.expected_codes
    assert len(set(codes)) == test_case.expected_unique_count
    assert "FFR206" not in codes
    assert RoleCode("FFR306").name == test_case.expected_sfr306_symbolic_name
    rules_by_code: dict[str, RuleSpec] = {rule.code: rule for rule in FFR_RULES}
    assert rules_by_code["FFR204"].message == test_case.expected_sfr204_message
    assert rules_by_code["FFR301"].execution_owner is test_case.expected_sfr301_owner
    assert rules_by_code["FFR302"].execution_owner is test_case.expected_sfr302_owner
    assert rules_by_code["FFR303"].slug == test_case.expected_sfr303_slug
    assert rules_by_code["FFR303"].message == test_case.expected_sfr303_message
    assert rules_by_code["FFR303"].remediation == test_case.expected_sfr303_remediation
    assert rules_by_code["FFR306"].slug == test_case.expected_sfr306_slug
    assert rules_by_code["FFR306"].message == test_case.expected_sfr306_message
    assert rules_by_code["FFR306"].remediation == test_case.expected_sfr306_remediation
    assert rules_by_code["FFR306"].execution_owner is test_case.expected_sfr306_owner
    assert rules_by_code["FFR307"].message == test_case.expected_sfr307_message
    assert rules_by_code["FFR307"].remediation == test_case.expected_sfr307_remediation
    assert rules_by_code["FFR308"].slug == test_case.expected_sfr308_slug
    assert rules_by_code["FFR308"].message == test_case.expected_sfr308_message
    assert rules_by_code["FFR308"].remediation == test_case.expected_sfr308_remediation
    assert rules_by_code["FFR308"].execution_owner is test_case.expected_sfr308_owner
    assert rules_by_code["FFR309"].slug == test_case.expected_sfr309_slug
    assert rules_by_code["FFR309"].message == test_case.expected_sfr309_message
    assert rules_by_code["FFR309"].remediation == test_case.expected_sfr309_remediation
    assert rules_by_code["FFR309"].execution_owner is test_case.expected_sfr309_owner
    assert rules_by_code["FFR706"].slug == test_case.expected_sfr706_slug
    assert rules_by_code["FFR706"].message == test_case.expected_sfr706_message
    assert rules_by_code["FFR706"].remediation == test_case.expected_sfr706_remediation
