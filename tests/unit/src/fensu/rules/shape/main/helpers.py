...
from tests.unit.src.fensu.rules.shape.main._test_types import ShapeRuleTestCase
...
def evaluate_shape_test_case(
    *, test_case: ShapeRuleTestCase, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ...
    rulesets_by_code: dict[str, tuple[RuleSpec, ...]] = {rule.code: (rule,) for rule in FFS_RULES}
    rulesets_by_code["FFS"] = FFS_RULES
