...
GOOD_TEST_SOURCE: str = (
    "import pytest\n\n"
    "from tests.unit.src.fensu.rules.tests.main._test_types import ExampleTestCase\n\n"
...
def evaluate_tests_rule_test_case(
    *, test_case: FftRuleTestCase, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> EvaluationResult:
    """Write a source tree and evaluate one tests rule."""

    _write_file(tmp_path=tmp_path, relative_path="src/fensu/__init__.py", source="")
    ...
    rulesets_by_code: dict[str, tuple[RuleSpec, ...]] = {rule.code: (rule,) for rule in FFT_RULES}
    rulesets_by_code["FF"] = CORE_RULES
...
def evaluate_configured_layout_test_case(
    *,
    ...
    layout_rules: tuple[RuleSpec, ...] = tuple(
        filter(lambda rule: "FFT001" <= rule.code <= "FFT008", FFT_RULES)
    )
    return evaluate(
        tree=discover_files(config=config),
...
def good_test_files(*, test_source: str = GOOD_TEST_SOURCE) -> tuple[FftRuleFile, ...]:
    ...
    return (
        ...
        FftRuleFile(
            ...
            relative_path="tests/unit/src/fensu/rules/tests/main/test_example.py",
            source=test_source,
        ),
    )
