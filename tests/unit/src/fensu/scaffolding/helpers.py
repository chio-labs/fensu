...
class ConcurrentGitIgnorePublisher:
    ...
    def __call__(self, *, repository: Path, plan: GitIgnorePlan) -> None:
        replacement: Path = self._path.with_name("concurrent-user-gitignore")
        replacement.write_bytes(self._user_content)
        _ = replacement.replace(self._path)
        self._publish(repository=repository, plan=plan)


class ConcurrentExclusiveOpener:
    """Create a commerce destination immediately before an exclusive open."""
    ...
    def __call__(
        ...
        *,
        ...
        race: Callable[[], None] = {
            ...
            True: self._publish_concurrent_destination,
        }[should_race]
        race()
    ...
    def _publish_concurrent_destination(self) -> None:
        for path, content in self._writes:
            path.write_bytes(content)
        for path, content in self._replacements:
            replacement: Path = path.with_name(f"concurrent-user-{path.name}")
...
class ConcurrentExclusiveLinker:
    """Create a commerce destination immediately before an exclusive hard link."""
...
class ConcurrentConfigStageLinker:
    ...
    def __call__(self, source: Path, target: Path, *, follow_symlinks: bool) -> None:
        replacement: Path = source.with_name(f"concurrent-user-{source.name}")
        replacement.write_bytes(self._user_content)
...
class ConcurrentDescriptorReader:
    ...
    def _replace_path(self) -> None:
        replacement: Path = self._path.with_name("concurrent-user-pyproject.toml")
...
def config_destination_value(*, root: Path) -> str:
    """Read the commerce destination without following symlinks unexpectedly."""
