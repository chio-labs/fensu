...
from fensu.scaffolding._helpers import capabilities as capabilities_module
from fensu.scaffolding._helpers import execution as execution_module
from fensu.scaffolding._helpers import gitignore as gitignore_module
from fensu.scaffolding._helpers.execution import build_rendered_config, execute_init_plan
from fensu.scaffolding._helpers.gitignore import is_gitignored, plan_gitignore_update
from fensu.scaffolding.constants import (
    FENSU_GITIGNORE_BLOCK,
...
from fensu.scaffolding.models import GitIgnorePlan, InitExecution, InitPlan
...
from tests.unit.src.fensu.scaffolding.helpers import (
    CountingFileOpener,
    FailingPublicationWriter,
    NoDirFdOpener,
    ConcurrentConfigStageLinker,
    ConcurrentExclusiveLinker,
    ConcurrentExclusiveOpener,
    ConcurrentGitIgnorePublisher,
...
def test_given_empty_repository_plan_when_executing_then_creates_exact_roundtrippable_scaffold(
    ...
    written_text: str = (tmp_path / "fensu.toml").read_text(encoding="utf-8")
    ...
    assert (tmp_path / ".gitignore").read_bytes() == (
        PYTHON_GITIGNORE_TEMPLATE + FENSU_GITIGNORE_BLOCK
    )
...
@pytest.mark.parametrize(
    ...
    [
        ExecutionFailureTestCase(
            ...
            expected_absent_paths=("fensu.toml", ".gitignore"),
...
def test_given_existing_config_path_when_executing_then_refuses_before_atomic_staging(
    ...
    assert lexisting_paths(root=tmp_path, paths=("fensu.toml",)) == ("fensu.toml",)
...
@pytest.mark.parametrize(
    ...
    [
        AtomicRaceTestCase(
            ...
            expected_destination_value="commerce config\n",
            expected_absent_paths=(),
        ),
        AtomicRaceTestCase(
            ...
            expected_destination_value="commerce-target",
            expected_absent_paths=("commerce-target",),
        ),
    ],
    ids=lambda case: case.description,
)
def test_given_concurrent_config_destination_when_publishing_then_never_overwrites_and_cleans_temp(
    test_case: AtomicRaceTestCase, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    build_repository(root=tmp_path, files=(("src/pkg/__init__.py", ""),))
    linker: ConcurrentExclusiveLinker = ConcurrentExclusiveLinker(
        root=tmp_path,
        destination_name="fensu.toml",
...
def test_given_replaced_config_stage_when_publishing_then_refuses_untrusted_content(
    ...
    linker: ConcurrentConfigStageLinker = ConcurrentConfigStageLinker(
...
@pytest.mark.parametrize(
    ...
    [
        ScaffoldSymlinkTestCase(
            ...
            expected_absent_paths=("fensu.toml", "outside/__init__.py"),
...
@pytest.mark.parametrize(
    ...
    [
        ParentSwapTestCase(
            ...
            expected_absent_paths=("outside/pkg/__init__.py", "fensu.toml", ".gitignore"),
        )
    ],
    ids=lambda case: case.description,
)
...
@pytest.mark.parametrize(
    ...
    [
        GitIgnoreExecutionTestCase(
            ...
            initial=None,
            expected_gitignore=FENSU_GITIGNORE_BLOCK,
        ),
        GitIgnoreExecutionTestCase(
...
def test_given_no_dirfd_capability_when_executing_brownfield_then_publishes_config_and_gitignore(
    ...
    assert (tmp_path / "fensu.toml").read_text(encoding="utf-8") == (
        'roots = ["src/pkg"]\ntests = ["tests"]\nselect = ["FF"]\n'
    )
...
def test_given_no_dirfd_capability_when_executing_greenfield_then_creates_complete_scaffold(
    ...
    assert execution.created_paths == test_case.expected_created_paths
    assert (tmp_path / "fensu.toml").read_text(encoding="utf-8") == test_case.expected_config_text
    assert (tmp_path / ".gitignore").read_bytes() == (
        PYTHON_GITIGNORE_TEMPLATE + FENSU_GITIGNORE_BLOCK
    )
    assert (
...
@pytest.mark.parametrize(
    ...
    [
        ScaffoldSymlinkTestCase(
            ...
            expected_error_type=InitError,
            expected_error_fragment="Refusing to scaffold through symlink path",
            expected_absent_paths=("outside/__init__.py", "fensu.toml", ".gitignore"),
            expected_symlink_paths=("src/pkg",),
        )
    ],
    ids=lambda case: case.description,
...
def test_given_selected_scope_python_symlink_when_executing_then_rejects_before_config(
    ...
    assert test_case.expected_error_fragment in str(error.value)
    assert (tmp_path / "fensu.toml").exists() is test_case.expected_config_present
...
@pytest.mark.parametrize(
    ...
    [
        GitIgnorePlanTestCase(
            description="greenfield without gitignore receives pinned Python template and Fensu block",
            initial=None,
            greenfield=True,
            expected_desired=PYTHON_GITIGNORE_TEMPLATE + FENSU_GITIGNORE_BLOCK,
        ),
        GitIgnorePlanTestCase(
            description="brownfield without gitignore receives only Fensu block",
            initial=None,
            greenfield=False,
            expected_desired=FENSU_GITIGNORE_BLOCK,
        ),
        GitIgnorePlanTestCase(
...
@pytest.mark.parametrize(
    ...
    [
        GitIgnoreMatcherTestCase(
            ...
            initial=b" .fensu/\n",
            relative_path=" .fensu/cache/v1.db",
...
@pytest.mark.parametrize(
    ...
    [
        GitIgnoreExecutionTestCase(
            description="brownfield without gitignore creates minimal Fensu block",
            initial=None,
            expected_gitignore=FENSU_GITIGNORE_BLOCK,
        ),
        GitIgnoreExecutionTestCase(
            description="brownfield uncovered gitignore appends Fensu block",
            initial=b"dist/\n",
            expected_gitignore=b"dist/\n" + FENSU_GITIGNORE_BLOCK,
        ),
        GitIgnoreExecutionTestCase(
...
def test_given_concurrent_gitignore_when_publishing_then_preserves_user_edit_and_rolls_back_init(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    ...
    publisher: ConcurrentGitIgnorePublisher = ConcurrentGitIgnorePublisher(
        path=gitignore,
        user_content=test_case.expected_destination_value.encode(),
    ...
    assert not (tmp_path / "fensu.toml").exists()
...
def test_given_concurrent_new_gitignore_when_publishing_then_preserves_user_file_and_rolls_back_scaffold(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    test_case: AtomicRaceTestCase,
) -> None:
    opener: ConcurrentExclusiveOpener = ConcurrentExclusiveOpener(
        root=tmp_path,
