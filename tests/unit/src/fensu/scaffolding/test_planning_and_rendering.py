...
from fensu.scaffolding._helpers.output import normalize_project_name
from fensu.scaffolding._helpers.planning import (
    build_init_plan,
...
from fensu.scaffolding.main._detect_repository_layout import detect_repository_layout
from fensu.scaffolding.main.find_local_config import find_local_config
from fensu.scaffolding.models import InitOptions, InitPlan
from fensu.scaffolding.types import InteractionDecision
...
@pytest.mark.parametrize(
    ...
    [
        LocalConfigCaptureTestCase(
            description="captured tool fensu survives concurrent pathname replacement",
            initial=b'[tool.fensu]\nroots = ["src/pkg"]\n',
            replacement=b'[project]\nname = "user-replacement"\n',
            expected_found=True,
        ),
        LocalConfigCaptureTestCase(
            description="captured plain pyproject ignores concurrent tool fensu replacement",
            initial=b'[project]\nname = "original"\n',
            replacement=b'[tool.fensu]\nroots = ["src/pkg"]\n',
            expected_found=False,
        ),
    ],
    ids=lambda case: case.description,
)
def test_given_concurrent_pyproject_path_when_finding_local_config_then_parses_captured_descriptor(
    tmp_path: Path,
    ...
    reader: ConcurrentDescriptorReader = ConcurrentDescriptorReader(
